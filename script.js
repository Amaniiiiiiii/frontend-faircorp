...
$.ajax({
  ...
  // url: 'http://localhost:8080/api/windows' ,
  dataType: 'json',
  success: function(data) {
      for (var i=0; i<data.length; i++) {
          var row = $('<tr><td>' + data[i].room.id+ '</td><td>' + data[i].id +  '</td><td>' + data[i].status + '</td><td>' + data[i].room.currentTemperature + '</td><td>' + data[i].room.targetTemperature + '</td><td>' + data[i].name+ '</td></tr>');
          $('#table').append(row);
      }
  },
